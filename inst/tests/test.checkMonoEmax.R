...
set.seed(12357)
p1nm<-suppressWarnings(checkMonoEmax(ynm,exdat$dose,parm,(sigsim)^2,
																		 trend='negative'))
...
p1<-suppressWarnings(checkMonoEmax(y1,dose1,parms,(sigsim)^2,
...
y1<-y[prot==1]
dose1<-dose[prot==1]
p1b<-suppressWarnings(checkMonoEmax(y1,dose1,parms,
								sigma2=numeric(length(y1)),xbase=x1,binary=TRUE))
