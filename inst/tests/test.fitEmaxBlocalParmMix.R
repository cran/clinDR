context('fiEmaxB with mixture pbo prior')
...
test_that("plot.fitEmaxB CI for binary data agree",{
	expect_that(clev,
							equals(as.numeric(mean(apply(covci,2,mean,na.rm=TRUE))),
										 tolerance=0.05,scale=1))
})
...
test_that("plot.fitEmaxB PI for binary data agree",{
	expect_that(clev,
							equals(as.numeric(mean(apply(covpi,2,mean,na.rm=TRUE))),
										 tolerance=0.05,scale=1))
})
...
test_that("plot.fitEmaxB CI DIF for binary data agree",{
	expect_that(clev,
							equals(as.numeric(mean(apply(covdifci[,-1],2,mean,na.rm=TRUE))),
										 tolerance=0.05,scale=1))
})
...
#### mixture PBO prior and covariates not currently implemented
####
# 
